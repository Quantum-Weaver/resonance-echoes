...
| 2026-08-21 | **KP's own sitting, not yet narrated here** — verified from git: version → **1.4.0** in every source (`package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `Cargo.lock` — commit `87c4218` "sidebar fixed-hamburger icon"). Docs reorganized: `docs/BUILD-SEQUENCE.md`, `CLAUDE-CONTEXT.md`, `FEATURE-BOARD.md`, `SCREEN-INVENTORY.md`, root `helper.md` moved to `docs/archive/*-2026-08-21.md`; `GALAXY-LISTING.md` and `PLAY-TRACK.md` moved under new `docs/listings/`, joined by `AUDHDITIES-LISTING.md` and `MSSTORE-LISTING.md` (commits `e30e2ce`, `87c4218`). Sidebar/hamburger + ComfortBar + theme store changes (`Sidebar.svelte`, `ComfortBar.svelte`, `theme.svelte.ts`, `theme/theme.ts`, `stores/ui.svelte.ts` — see `87c4218` for the full 19-file diff). `HANDOFF.md` created and `src/lib/cumdach/index.ts` extended (commit `26093d8`, "cumdach updated"). **No CHECKLIST phase/bug row cites this work and no device-verification note exists for 1.4.0** — the 07-21/08-16 precedent (badge follows the device, not the files) is not yet satisfied, so the README/CLAUDE version badges were left at **1.3.2** rather than bumped blind; see CONFUSIONS in this sitting's repo-tender report. |
| 2026-08-21 | **Repo-tender sweep** (badges · story block · checklist, per KP's ask) — `README.md`: `## Development Standards` line trued to read "Sanctuary Standards" (was "Resonance Standards", verbatim mismatch against the standard); the inline Story Block renamed `## Story Block` → `## THE STORY`, given the standard's required reference line, shortened to a teaser, and pointed at a new `docs/STORY-BLOCK.md` (nothing deleted — the full block re-homed there verbatim); the `FOR DEVELOPERS` tree trued to the disk (`src/lib/cumdach`, `epagoge`, `sky`, `theme`, `routes/sattva`, `routes/timer` were missing, confirmed present via `find src`). `docs/STORY-BLOCK.md` **created** (was absent) — 11 sections per the standard's template, content carried over from the README's prior inline block; ETYMOLOGY newly added, citing the same 1996-song fact already on record (no second source). This row itself. Version badge (1.3.2) left untouched — see the row above and CONFUSIONS. |
---
