...
### v1.3.0: Sovereignty Whole + The Timer That Waits 🔵 (2026-07-26 — code complete, build/sign/install pending KP's hands)
...
- [ ] **Sign (KP's hand):** `python sign-release.py resonance-echoes` → artifacts to `release/` → `install-app.py` both phones → device-verify: export past 200 · envelope round-trip · pause across navigation · each chime at volume. README badge bumps only on his phone's word (the 07-21 precedent).
